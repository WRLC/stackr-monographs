...
wc_get_token <- function() {
  ...
  tryCatch(
          req <- POST('https://oauth.oclc.org/token',
  ...
  if(nrow(new) > 0) {
    ...
    tmp <- sapply(seq_along(new$oclc_num),
                  function(i) {
                    ...
                    s <- Sys.time()
                    attr(s, "tzone") <- "UTC"
                    if(token$expires_at <= format(s, "%Y-%m-%d %H:%M:%SZ")) {
                      # If it is, refresh it
                      token <- refresh_wc_token(token)
                    } else {
                      token <- token
                    }
...
renderValueBox({
  ...
  valueBox(
    ...
    icon = "fa-university",
    color = "#076293",
    caption = "Total number of monograph records (by network number) in the SCF"
  )
